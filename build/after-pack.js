...
function resolveRcedit(projectDir) {
  ...
  if (!hit) throw new Error('No usable rcedit executable was found for 52MUSIC icon injection.');
  return hit;
}
...
module.exports = async function afterPack(context) {
  ...
  const appName = context.packager.appInfo.productFilename || '52MUSIC';
  ...
  if (!fs.existsSync(exePath)) throw new Error(`52MUSIC executable was not found: ${exePath}`);
  if (!fs.existsSync(iconPath)) throw new Error(`52MUSIC icon was not found: ${iconPath}`);

  const version = context.packager.appInfo.version;
  console.log(`  • injecting 52MUSIC resources  rcedit=${rceditPath}`);
  execFileSync(rceditPath, [
    exePath,
    '--set-icon', iconPath,
    '--set-version-string', 'FileDescription', '52MUSIC',
    '--set-version-string', 'ProductName', '52MUSIC',
    '--set-version-string', 'CompanyName', '52MUSIC',
